...
export function AgendarModal({
    ...
    const ultimoElementoFocado = useRef<HTMLElement | null>(null);

    useEffect(() => {
        if (!aberto) return;

        ultimoElementoFocado.current = document.activeElement as HTMLElement;

        const modal = modalRef.current;

        if (!modal) return;

        const elementosFocaveis = modal.querySelectorAll<HTMLElement>(
        ...
        return () => {
            ...
            ultimoElementoFocado.current?.focus();
        };
    }, [aberto, onFechar]);
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="relative z-10 px-7 pb-7 pt-7 pr-14 md:px-9 md:pb-9 md:pt-9 md:pr-16">
                                <motion.button
                                    type="button"
                                    onClick={onFechar}
                                    aria-label="Fechar modal"
                                    className="absolute right-5 top-5 cursor-pointer text-zinc-400 transition duration-300 hover:text-[#A2E317] focus:outline-none focus:ring-2 focus:ring-[#A2E317]"
                                ...
                                >
                                    <motion.a
                                        ...
                                        className="flex items-center justify-center bg-white px-6 py-3 text-center text-xs font-black uppercase tracking-[0.22em] text-black transition duration-300 hover:border-[#A2E317] hover:bg-[#A2E317] focus:outline-none focus:ring-2 focus:ring-[#A2E317]"
                                        style={{
                                            clipPath:
                                    ...
                                    <motion.a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex items-center justify-center bg-[#A2E317] px-6 py-3 text-center text-xs font-black uppercase tracking-[0.22em] text-black transition duration-300 hover:brightness-110 focus:outline-none focus:ring-2 focus:ring-white"
                                        style={{
                                            clipPath:
                                                "polygon(10% 0, 100% 0, 100% 82%, 90% 100%, 0 100%, 0 18%)",
                                        }}
